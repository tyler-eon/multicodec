defmodule Multicodec do
  @moduledoc """
  The primary module for fetching multicodec metadata and the central registry
  for the multiformats ecosystem.

  Multicodec serves as the composition layer that ties together the various
  multiformats components (multibase, multihash, CID, etc.). Each component
  has its own self-contained library for encoding/decoding, but multicodec
  provides the shared codec identifier table and a runtime registry where
  external libraries can register their codec handlers.

  ## Built-in codecs

  The built-in codec table is generated from
  [this source table](https://github.com/multiformats/multicodec/blob/master/table.csv).
  The mix task `mix multicodec.gen` can regenerate it from any
  appropriately-formatted CSV file.

  ## Runtime registry

  The codec table is fixed at compile time — external libraries cannot
  define new codecs at runtime. However, they can register **handler
  modules** for existing codecs via `Multicodec.Registry.register/2`.
  This lets higher-level libraries dispatch encode/decode calls through
  a central entrypoint.

  See `Multicodec.Registry` for details.
  """
